...
namespace DotNetWikiBot
{
	...
	public class Site
	{
		...
		/// This function allows distributing compiled bots without password revealing.</summary>
		...
		private void LoadGeneralInfo()
		{
			...
			namespaces = (
				...
				select new {
					...
					name = ('|' + el.Value + '|' + el.Attribute("canonical").Value + '|').ToString()
				}
			...
			if (generalData.ContainsKey("generator")) {
				...
				double.TryParse(Regex.Match(generalData["generator"], @"\d+\.\d+").Value,
					NumberStyles.AllowDecimalPoint, new CultureInfo("en-US"), out versionNumber);
			...
			string allNamespaces = string.Join("",
				namespaces.Select(x => x.Value.Substring(1, x.Value.Length-2)).ToArray());
			regexes["allNamespaces"] = new Regex(@"(?i)^(?:" + allNamespaces + "):");
		...
		/// <summary>Gets the list of all WikiMedia Foundation wiki sites as listed on
		/// http://meta.wikimedia.org/wiki/Special:SiteMatrix </summary>
		...
		public List<string> GetWikimediaProjects(bool officialOnly)
		{
			...
			MatchCollection siteMatches = siteRegex.Matches(src);
			return siteRegex.Matches(src).Cast<Match>().Select(m => m.Groups[1].Value).ToList();
		}

		/// <summary>This internal function gets the text of page from web.</summary>
			...
			for (int errorCounter = 0; true; errorCounter++) {
				...
				try {
					...
					if (Regex.IsMatch(message, ": \\(50[02349]\\) ")) {    // Remote problem
						...
						//Console.Write("|");
						return PostDataAndGetResult(pageURL, postData, getCookies,
							allowRedirect);
					}
					else
						throw;
		...
		/// without parameters, e.g. http://en.wikipedia.org/w/api.php, API Sandbox is also very
		/// useful for experiments: http://en.wikipedia.org/wiki/Special:ApiSandbox.
		/// Parameters' values must be URL-encoded with HttpUtility.UrlEncode() function
		...
		/// 	"cmnamespace=0|14&cmcategory=" + HttpUtility.UrlEncode("Physical sciences"),
		/// 	int.MaxValue);
		/// </code></example>
		/// <example><code>
		/// GetApiQueryResult("list=logevents",
		/// 	"letype=patrol&titles=" + HttpUtility.UrlEncode("Physics"),
		/// 	200);
		/// </code></example>
		/// <example><code>
		/// GetApiQueryResult("prop=links",
		/// 	"titles=" + HttpUtility.UrlEncode("Physics"),
		/// 	int.MaxValue);
		/// </code></example>
		...
		/// <summary>Gets local canonical prefix for specified namespace and colon.</summary>
		/// <param name="nsIndex">Index of namespace to get prefix for.</param>
		/// <returns>Returns the canonical prefix with colon.</returns>
		...
		/// <summary>Gets English default prefix for specified namespace and colon.
		/// If default prefix is not found the local canonical prefix is returned.</summary>
		/// <param name="nsIndex">Index of namespace to get prefix for.</param>
		/// <returns>Returns the prefix with colon.</returns>
		...
		public string RemoveNsPrefix(string pageTitle, int nsIndex)
		{
			...
			string pageNS = '|' + pageTitle.Substring(0, colonPos) + '|';
			if (nsIndex != 0) {
				if (namespaces[nsIndex].Contains(pageNS))
	...
	public class Page
	{
		...
		public void Load()
		{
			...
			string res = site.indexPath + "?title=" + HttpUtility.UrlEncode(title) +
		...
		/// <summary>Loads page text and metadata.
		/// It is slower than Load(), it should be used if some page's metadata is required
		/// (revision ID, timestamp, last comment, last contributor, minor edit mark).</summary>
		public void LoadWithMetadata()
		{
			if (site.useApi) {
				...
				if (!string.IsNullOrEmpty(title))
					res += "&titles=" + HttpUtility.UrlEncode(title);
		...
		public Dictionary<string, string> GetSecurityTokens(string action)
		{
			...
			string src = site.GetWebPage(site.apiPath + "?action=query&prop=info&intoken=" +
				...
				"&format=xml&titles=" + HttpUtility.UrlEncode(title));
		...
		/// edit comment and default minor edit mark setting (true).</summary>
		public void Save()
		{
		...
		{
			if (string.IsNullOrEmpty(title))
				throw new WikiBotException(Bot.Msg("No title is specified for page to save text to."));
			if (string.IsNullOrEmpty(newText) && string.IsNullOrEmpty(text))
				throw new WikiBotException(Bot.Msg("No text is specified for page to save."));
			...
			// Firstly get security tokens for editing
			...
			string editStartTime = tokens.ContainsKey("starttimestamp")
				? Regex.Replace(tokens["starttimestamp"], "\\D", "")
				: DateTime.Now.ToUniversalTime().ToString("yyyyMMddHHmmss");
			string lastRevisionTime = editStartTime;
			if (timestamp != null && timestamp != DateTime.MinValue)
				lastRevisionTime = timestamp.ToString("yyyyMMddHHmmss");
			else if (tokens.ContainsKey("touched"))
				lastRevisionTime = Regex.Replace(tokens["touched"], "\\D", "");

			string postData = string.Format(
				"wpSection=&wpStarttime={0}&wpEdittime={1}&wpScrolltop=&wpTextbox1={2}" +
				"&wpSummary={3}&wpSave=Save%20Page&wpEditToken={4}{5}{6}{7}",
				editStartTime,
				lastRevisionTime,
				HttpUtility.UrlEncode(newText),
				HttpUtility.UrlEncode(comment),
				HttpUtility.UrlEncode(tokens["edittoken"]),
			...
			DateTime expiryDate, string reason)
		{
			...
			if (site.useApi) {
				...
				string postData = string.Format("token={0}&protections=edit={1}|move={2}" +
					...
					HttpUtility.UrlEncode(tokens["protecttoken"]),
				...
				string respStr = site.PostDataAndGetResult(site.apiPath + "?action=protect" +
					"&title=" + HttpUtility.UrlEncode(title) + "&format=xml", postData);
				...
				string respStr = site.GetWebPage(site.indexPath + "?title=" +
					HttpUtility.UrlEncode(title) + "&action=protect");
				...
				respStr = site.PostDataAndGetResult(site.indexPath +
					"?title=" + HttpUtility.UrlEncode(title) + "&action=protect", postData);
		...
		public void Watch()
		{
			...
			if (site.useApi) {
				...
				string postData = string.Format("titles={0}{1}&action=watch&token={2}&format=xml",
					HttpUtility.UrlEncode(title), HttpUtility.UrlEncode(titleFallback),
					HttpUtility.UrlEncode(securityToken));
				...
				string postData = string.Format("title={0}&action=watch&wpEditToken={1}",
					HttpUtility.UrlEncode(title), HttpUtility.UrlEncode(securityToken));
				respStr = site.PostDataAndGetResult(site.indexPath +
					"?title=" + HttpUtility.UrlEncode(title), postData);
			}
		...
		public void Unwatch()
		{
			...
			if (site.useApi) {
				...
				string postData = string.Format("titles={0}{1}&token={2}" +
					"&format=xml&action=watch&unwatch=1", HttpUtility.UrlEncode(title),
					HttpUtility.UrlEncode(titleFallback), HttpUtility.UrlEncode(securityToken));
				respStr = site.PostDataAndGetResult(site.apiPath, postData);
			}
			else {
			...
			string license, string copyStatus)
		{
			if (string.IsNullOrEmpty(imageFileUrl))
				throw new WikiBotException(Bot.Msg("No URL specified for image to upload."));
		...
		/// Load() or LoadWithMetadata() before using this function.</summary>
		/// <returns>Returns bool value.</returns>
		public bool Exists()
		{
		...
		/// different languages. But in order to be used on other sites, Site.disambig
		/// variable must be manually set before this function is called. Site.disambig
		/// should contain local disambiguation template's title or several titles delimited
		/// by '|' character, letters case doesn't matter, e.g. "disambiguation|disambig|disam".
		/// Page contents must be loaded from site before this function is called. Page's text
		/// will be loaded if it was not loaded prior to function call.</summary>
		/// <returns>Returns bool value.</returns>
		public bool IsDisambig()
		{
			...
			string src2 = site.GetWebPage(site.apiPath + "?format=xml&action=query" +
				...
				HttpUtility.UrlEncode(disambigTemplate));
		...
		/// <returns>Returns List object. Each string contains page title 
		...
		/// strings to call FillFromCategory() or FillFromCategoryTree()
		...
		public List<string> GetAllCategories()
		{
			...
			if (site.useApi) {
				...
				uri = site.indexPath + "?title=" + HttpUtility.UrlEncode(title) + "&redirect=no";
			...
			XPathNodeIterator iterator = Bot.GetXMLIterator(src, xpathQuery, site.xmlNs);
			List<string> matchStrings = new List<string>();
			iterator.MoveNext();
		...
		/// curly brackets {{...}} ). MediaWiki's "magic words"
		/// (https://www.mediawiki.org/wiki/Help:Magic_words) are not returned as templates.
		/// </summary>
		/// <param name="withParameters">If set to true, everything inside curly brackets is
		/// returned with all parameters untouched.</param>
		/// <param name="includePages">If set to true, "transcluded" pages titles are returned as
		/// templates and messages with "msgnw:" prefix are also returned as templates. See
		/// https://www.mediawiki.org/wiki/Transclusion for details. Default is false.</param>
		/// <returns>Returns the List object.</returns>
		public List<string> GetTemplates(bool withParameters, bool includePages)
		{
			...
			foreach (KeyValuePair<int, int> pos in templPos) {
				...
				if (!includePages) {
					if (site.regexes["allNamespaces"].IsMatch(str)
						|| str[0] == ':'
		...
		public void RenameTo(string newTitle, string reason)
		{
			...
			if (site.useApi) {
				...
				string postData = string.Format("from={0}&to={1}&reason={2}&token={3}",
					HttpUtility.UrlEncode(title),
					HttpUtility.UrlEncode(newTitle),
					HttpUtility.UrlEncode(reason),
					HttpUtility.UrlEncode(tokens["movetoken"]));
		...
		public void Delete(string reason)
		{
			...
			if (site.useApi) {
				...
				string postData = string.Format("reason={0}&token={1}",
					HttpUtility.UrlEncode(reason), HttpUtility.UrlEncode(tokens["deletetoken"]));
				...
				string postData = string.Format("wpReason={0}&wpEditToken={1}",
					HttpUtility.UrlEncode(reason), HttpUtility.UrlEncode(securityToken));
	...
	public class PageList
	{
		...
		/// <summary>This function allows to iterate over Page objects in this PageList
		/// using "foreach" loop.</summary>
			...
			if (site.useApi) {
				FillFromCustomApiQuery("list=allpages", "apnamespace=" + neededNSpace +
					...
					(string.IsNullOrEmpty(firstPageTitle) ? "" : "&apfrom=" +
						HttpUtility.UrlEncode(firstPageTitle)), limit);
			}
			else {
				Console.WriteLine(
				...
				MatchCollection matches;
				do {
					string res = site.indexPath + "?title=Special:Allpages&from=" +
		...
		/// The following special pages are supported by now:<b><i>
			...
			if (site.useApi) {
				...
				if (fallback) {    // FALLBACK, use alternative parsing way, XPath
					...
					XPathNodeIterator ni = Bot.GetXMLIterator(src,
						"//ns:ol/ns:li/ns:a[@title != '']", site.xmlNs);
					if (ni.Count == 0)
						throw new WikiBotException(string.Format(
			...
			int limit)
		{
			...
			if (site.useApi) {
				...
				if (!string.IsNullOrEmpty(userName))
					parameters += "&leuser=" + HttpUtility.UrlEncode(userName);
				if (!string.IsNullOrEmpty(pageTitle))
					parameters += "&letitle=" + HttpUtility.UrlEncode(pageTitle);
			...
			Console.WriteLine(Bot.Msg("PageList has been filled with {0} page titles from \"{1}\" log."),
		...
		public void FillFromCustomApiQuery(string query, string queryParams, int limit)
		{
			...
			if (!string.IsNullOrEmpty(Environment.StackTrace)
				...
				Console.WriteLine(string.Format(Bot.Msg("PageList has been filled with {0} page " +
		...
		/// not included, call FillAllFromCategory function instead to get category contents with
		...
		/// FillAllFromCategoryTree function instead to get PageList with subcategories on board.
		...
		public void FillFromPageHistory(string pageTitle, int limit)
		{
			...
			Console.WriteLine(Bot.Msg("PageList has been filled with {0} last revisons of \"{1}\" page..."),
		...
		public void FillFromLinksToPage(string pageTitle)
		{
			...
			FillFromCustomApiQuery("list=backlinks",
				"bltitle=" + HttpUtility.UrlEncode(pageTitle), int.MaxValue);
			Console.WriteLine(
		...
		public void FillFromTransclusionsOfPage(string pageTitle)
		{
			...
			if (site.useApi) {
				FillFromCustomApiQuery("list=embeddedin", "eititle=" +
					HttpUtility.UrlEncode(pageTitle), int.MaxValue);
			}
			else {    // TO DO: paging
				string res = site.indexPath + "?title=Special:Whatlinkshere/" +
					HttpUtility.UrlEncode(pageTitle) + "&limit=5000&hidelinks=1&hideredirs=1";
				string src = site.GetWebPage(res);
				src = Bot.GetSubstring(src,
		...
		public void FillFromPagesUsingImage(string imageFileTitle)
		{
			...
			if (site.useApi) {
				FillFromCustomApiQuery("list=imageusage", "iutitle=" +
					HttpUtility.UrlEncode(site.GetNsPrefix(6)) +
					HttpUtility.UrlEncode(imageFileTitle), int.MaxValue);
			}
			else {    // TO DO: paging
				string res = site.indexPath + "?title=" +
					HttpUtility.UrlEncode(site.GetNsPrefix(6)) +
		...
		public void FillFromUserContributions(string userName, int limit)
		{
			...
			string res = site.indexPath +
				"?title=Special:Contributions&target=" + HttpUtility.UrlEncode(userName) +
		...
		public void FillFromChangedWatchedPages()
		{
			...
			foreach (Match match in matches)
				pages.Add(new Page(site, HttpUtility.HtmlDecode(match.Groups[1].Value)));
			Console.WriteLine(
				Bot.Msg("PageList has been filled with changed pages from bot account's watchlist."));
		}
		...
		public void FillFromSearchResults(string searchStr, int limit)
		{
			...
			if (site.useApi) {
				FillFromCustomApiQuery("list=search", "srsearch=" +
					HttpUtility.UrlEncode(searchStr), limit);
			}
			else {    // TO DO: paging
		...
		public void FillFromFile(string filePathName)
		{
			...
			strmReader.Close();
			Console.WriteLine(
				Bot.Msg("PageList has been filled with titles found in \"{0}\" file."), filePathName);
		}
		...
		/// framework errors may arise in case of overloading).</summary>
		/// <param name="comment">Your edit comment.</param>
		/// <param name="minorEditByDefault">Minor edit mark (true = minor edit).</param>
		public void Save(string comment, bool isMinorEdit)
		{
			foreach (Page page in pages)
		...
		/// between each page save operation in order not to overload server. Uses default Site's
		/// edit comment and default minor edit mark setting.</summary>
		public void SaveSmoothly()
		{
			SaveSmoothly(5, site.defaultEditComment, site.minorEditByDefault);
		}
		...
		/// Uses default Site's edit comment and default minor edit mark setting.</summary>
		...
		/// call LoadWithMetadata() or FillAndLoadFromXmlDump() to do that.
		...
		/// forget to load the pages from site using pageList.LoadWithMetadata().</summary>
	...
	public class Bot
	{
		...
		public static readonly Version version = new Version("3.11");
		/// <summary>Local cache directory.</summary>
		public static string cacheDir;
		/// <summary>Last Site object constructed by framework.</summary>
		public static Site lastSite = null;
		/// <summary>Some supplementary data.</summary>
		public static XElement commonDataXml;
		/// <summary>Desired bot's console messages language (ISO 639-1 language code).
		...
		/// False by default. Set it to true if you would like to.</summary>
		/// <example><code>Bot.askConfirm = true;</code></example>
		public static bool askConfirm = false;
		/// <summary>If true, bot reports errors and warnings only. Call EnableSilenceMode
		...
		/// <summary>The destructor is used to uninitialize Bot object.</summary>
		~Bot()
		{
		...
		/// <summary>Gets most recent Site object constructed by DotNetWikiBot Framework.</summary>
		...
		public static string GetSubstring(string src, string startMark, string endMark)
		{
			return GetSubstring(src, startMark, endMark, false, false, true);
		}
		...
		/// <summary>This internal helper function deletes everything before startTag and everything
		/// after endTag in the provided XML/XHTML source code and then inserts back the deleted
		/// DOCTYPE definition and root element of XML/XHTML document.</summary>
		...
		/// <summary>This internal function removes all attributes of root XML/XHTML element
		...
		/// <summary>This internal function constructs XPathDocument object, makes XPath query and
		...
		/// <summary>This internal function constructs XmlReader object
		...
		/// traffic compression and it lacks other special features.</summary>
